...
(g! fzf_tags_command
    "ctags -R --languages=python --exclude=node_modules --exclude=.venv")
...
; Use system clipboard. Very slow under WSL.
...
;; Fix inode Docker issues https://github.com/moby/moby/issues/15793#issuecomment-161162118
; (set! spell true)
(set! spelllang :en_gb)
             ...
             (when (and (vim.api.nvim_buf_get_option b :modified)
               ...
               (when (= (vim.fn.getbufvar b :&buftype) "")
                 (when (not= (vim.fn.bufname) "")
                   ...
                   (print (string.format "Saved: %s %s" (vim.fn.expand "%:t")
                                         (os.date "%X")))))))])
